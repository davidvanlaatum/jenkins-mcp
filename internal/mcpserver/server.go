...
const (
	...
	serverInstructions = "Call jenkins_get_capabilities before other tools to discover configured controllers, response limits, optional Jenkins plugin capabilities, mutation availability, and update notices. " +
		...
		"For jenkins_watch_build and jenkins_watch_queue_item, prefer omitting waitTimeoutMs to use the configured default (2 minutes), or use longer waits such as 300000 ms when supported by the host (configured maximum defaults to 15 minutes). Shorten waits only for a known shorter host deadline or an observed host timeout; do not assume a 30-second limit. Watches return early on relevant changes. Pass the returned short state ID as lastState on subsequent calls. Build watches default to waiting for completion or required input; use waitFor=change for stage transitions. IDs require re-bootstrap after cache idle expiry, eviction, or server restart. Timeout responses omit snapshots; continue with the returned state ID."
)
...
func (s *Server) register() {
	...
	addConfiguredTool(s, readOnlyTool("jenkins_watch_build", "Watch Build", "Bootstrap or long-poll a Jenkins build watcher. The first call without lastState returns immediately with a short server-held watch.state ID; pass that state back as lastState on later calls to wait until completion or required Pipeline input (default waitFor=completion_or_input); use waitFor=change to also wake on Pipeline stage-status or pending input changes. Timeout responses omit build and pipeline snapshots; continue with the returned ID. Invalid or expired lastState values return an error and must be re-bootstrapped. Prefer this over jenkins_get_build when waiting for progress."), func(ctx context.Context, in jenkinstools.WatchBuildRequest) (jenkinstools.WatchBuildResponse, error) {
	...
	addConfiguredTool(s, readOnlyTool("jenkins_watch_queue_item", "Watch Queue Item", "Bootstrap or long-poll a Jenkins queue item watcher. The first call without lastState returns immediately with a short server-held watch.state ID; pass that state back as lastState on later calls to wait until stable queue item fields change, the item receives an executable build, is cancelled, disappears from the queue, or reaches waitTimeoutMs. Jenkins why text changes, such as quiet-period countdowns, do not wake the long poll by themselves. Timeout responses omit item and build snapshots; continue with the returned ID."), func(ctx context.Context, in jenkinstools.WatchQueueItemRequest) (jenkinstools.WatchQueueItemResponse, error) {
