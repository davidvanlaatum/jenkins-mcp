...
func (s *Server) register() {
	...
	addConfiguredTool(s, readOnlyTool("jenkins_list_jobs", "List Jobs", "List Jenkins jobs at the controller root or within a folder. Supports cursor pagination, recursive traversal, and filters for case-insensitive nameContains, nameRegex, type, derived status, building state, and lastCompletedBuild JUnit summaries via hasTests, hasFailedTests, and hasSkippedTests."), func(ctx context.Context, in jenkinstools.ListJobsRequest) (jenkinstools.ListJobsResponse, error) {
