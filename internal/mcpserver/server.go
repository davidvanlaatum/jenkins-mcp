...
func (s *Server) register() {
	...
	addConfiguredTool(s, readOnlyTool("jenkins_get_flaky_test_stats", "Get Flaky Test Stats", "Analyze compact JUnit test status histories across selected builds for one job. Supports recent completed builds, explicit build numbers, build number ranges, result filtering, minTransitions, bounded parallelism, and a returned-test limit applied after sorting by transition count. Builds with no JUnit report are ignored, missing per-test observations do not count as transitions, and failedBuilds are references only for targeted jenkins_get_test_report follow-up."), func(ctx context.Context, in jenkinstools.FlakyTestStatsRequest) (jenkinstools.FlakyTestStatsResponse, error) {
