...
func TestListJobsToolDescriptionAndInputSchemaMentionFilters(t *testing.T) {
	...
	for tool, err := range clientSession.Tools(ctx, nil) {
		...
		for _, want := range []string{"cursor", "nameContains", "nameRegex", "type", "status", "buildable", "building", "hasLastBuild", "lastBuildAfter", "lastCompletedBuildBefore", "hasLastSuccessfulBuild", "lastFailedBuildAfter"} {
