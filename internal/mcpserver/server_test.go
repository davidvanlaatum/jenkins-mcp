...
func TestRegisteredToolAnnotations(t *testing.T) {
	...
	want := map[string]wantToolAnnotations{
		...
		"jenkins_get_replay_scripts":    {readOnly: true},
...
func TestRegisteredToolTitles(t *testing.T) {
	...
	want := map[string]string{
		...
		"jenkins_get_replay_scripts":    "Get Replay Scripts",
		...
		"jenkins_replay_build":          "Replay Build",
...
func TestListJobsToolDescriptionAndInputSchemaMentionFilters(t *testing.T) {
	...
	for tool, err := range clientSession.Tools(ctx, nil) {
		...
		for _, want := range []string{"cursor", "nameContains", "nameRegex", "type", "status", "buildable", "building", "hasLastBuild", "hasWarningsNgIssues", "hasCoverage", "lastBuildAfter", "lastCompletedBuildBefore", "hasLastSuccessfulBuild", "lastFailedBuildAfter", "hasTests", "hasFailedTests", "hasSkippedTests"} {
...
func TestListBuildsToolDescriptionAndInputSchemaMentionFilters(t *testing.T) {
	r := require.New(t)
	cfg := config.Config{
		DefaultController: "default",
		Controllers:       []config.ControllerConfig{{ID: "default", URL: "https://jenkins.example.com"}},
	...
	wantFilters := []string{"cursor", "result", "building", "completed", "startedAfter", "startedBefore", "durationMillisMin", "durationMillisMax", "estimatedDurationMillisMin", "estimatedDurationMillisMax", "keepLog", "queueId", "numberMin", "numberMax", "descriptionContains", "displayNameContains"}
	for tool, err := range clientSession.Tools(ctx, nil) {
		r.NoError(err, "Tools()")
		if tool.Name != "jenkins_list_builds" {
			continue
		}
