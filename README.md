...
- `jenkins_list_jobs`: List Jenkins jobs at the controller root or within a folder, with cursor pagination, optional recursive traversal, and filters for name, type, status, and building state. Job items include buildable status. Responses use `items`, `nextCursor`, `hasMore`, `truncated`, and `limit`.
